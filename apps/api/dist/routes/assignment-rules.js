...
exports.evaluateAssignmentRules = evaluateAssignmentRules;
exports.assignmentRuleRoutes = assignmentRuleRoutes;
...
function validateConditions(conditions) {
    ...
    for (const key of Object.keys(cond)) {
        ...
        if (typeof cond[key] !== 'string')
            throw new Error(`Condition "${key}" must be a string value`);
    }
    if (cond.subjectMatchesRegex) {
        // Validate the regex compiles — prevents obviously broken patterns
        try {
            new RegExp(cond.subjectMatchesRegex);
        }
        catch (e) {
            throw new Error(`Invalid regex in subjectMatchesRegex: ${e.message}`);
        }
    }
}
/**
 ...
 */
async function evaluateAssignmentRules(mailboxId, subject, senderEmail) {
    const rules = await prisma_js_1.prisma.assignmentRule.findMany({
...
async function assignmentRuleRoutes(fastify) {
    ...
    fastify.get('/', async (request) => {
        ...
        return prisma_js_1.prisma.assignmentRule.findMany({
    ...
    fastify.post('/', async (request, reply) => {
        const { mailboxId, priority = 0, conditions, assignToUserId, assignToGroupId, isActive = true } = request.body;
        if (!mailboxId)
            return reply.status(400).send({ error: 'mailboxId is required' });
        if (!conditions)
            return reply.status(400).send({ error: 'conditions is required' });
        ...
        const rule = await prisma_js_1.prisma.assignmentRule.create({
    ...
    fastify.patch('/:id', async (request, reply) => {
        ...
        const { priority, conditions, assignToUserId, assignToGroupId, isActive } = request.body;
        ...
        const rule = await prisma_js_1.prisma.assignmentRule.update({
            where: { id },
    ...
    fastify.delete('/:id', async (request, reply) => {
        const { id } = request.params;
        await prisma_js_1.prisma.assignmentRule.delete({ where: { id } }).catch(() => { });
        return { success: true };
    });
}
