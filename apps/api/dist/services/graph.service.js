 ...
 */
async function graphRequest(fn, retries = 4) {
    let lastErr;
...
class GraphService {
     ...
     */
    async fetchMessagesDelta(mailboxEmail, deltaLink) {
        ...
        while (nextLink) {
            const response = await graphRequest(() => client.api(nextLink)
     ...
     */
    async createSubscription(mailboxEmail) {
        const client = await this.getClient();
    ...
    async renewSubscription(subscriptionId) {
        ...
        return graphRequest(() => client.api(`/subscriptions/${subscriptionId}`)
     ...
     */
    async sendReply(fromMailboxEmail, toAddress, subject, bodyHtml, replyToMessageId, cc, bcc) {
        const client = await this.getClient();
        if (replyToMessageId) {
            // Step 1 – create a draft reply (sets threading headers automatically)
            const draft = await graphRequest(() => client.api(`/users/${fromMailboxEmail}/messages/${replyToMessageId}/createReply`).post({}));
            ...
            if (cc?.length)
                patch.ccRecipients = cc.map(addr => ({ emailAddress: { address: addr } }));
            if (bcc?.length)
                patch.bccRecipients = bcc.map(addr => ({ emailAddress: { address: addr } }));
            await graphRequest(() => client.api(`/users/${fromMailboxEmail}/messages/${draft.id}`).patch(patch));
            // Step 3 – send the draft (Graph automatically saves a copy to Sent Items)
            await graphRequest(() => client.api(`/users/${fromMailboxEmail}/messages/${draft.id}/send`).post({}));
        }
        else {
            ...
            await graphRequest(() => client.api(`/users/${fromMailboxEmail}/sendMail`).post({ message, saveToSentItems: true }));
        }
    }
    /**
     ...
     */
    async sendNewEmail(fromMailboxEmail, to, cc, subject, bodyHtml, bcc) {
        const client = await this.getClient();
        const message = {
            subject,
            body: { contentType: 'HTML', content: bodyHtml },
     ...
     */
    async sendMail(fromMailboxEmail, toAddress, subject, bodyText) {
        const client = await this.getClient();
        await graphRequest(() => client.api(`/users/${fromMailboxEmail}/sendMail`).post({
            message: {
                subject,
     ...
     */
    async fetchAttachment(mailboxEmail, messageId, attachmentId) {
        const client = await this.getClient();
        const response = await graphRequest(() => client.api(`/users/${mailboxEmail}/messages/${messageId}/attachments/${attachmentId}`).get());
     ...
     */
    async searchMessages(mailboxEmail, searchQuery) {
        const client = await this.getClient();
        // We escape double quotes in the query to avoid breaking the OData filter
        const escapedQuery = searchQuery.replace(/"/g, '\\"');
        const response = await graphRequest(() => client.api(`/users/${mailboxEmail}/messages`)
            .search(`"${escapedQuery}"`)
     ...
     */
    async getMessage(mailboxEmail, messageId) {
        const client = await this.getClient();
        return graphRequest(() => client.api(`/users/${mailboxEmail}/messages/${messageId}`)
            .select('id,subject,body,from,toRecipients,parentFolderId,conversationId,hasAttachments,receivedDateTime,sentDateTime,internetMessageId,internetMessageHeaders')
            .expand('attachments($select=id,name,contentType,size,isInline)')
            .get());
    }
